...
- **Speed:** 18ms per character (≈55 chars/sec — fast enough that long replies don't drag, slow enough that the char-by-char reveal still reads as a typewriter). Originally specified as 45ms (Disco Elysium pacing); dialled down during Task 5 manual QA in two passes (45 → 25 → 18). See §12 row 5 and §14 for the deviation history.
...
- A timer (set up via `requestAnimationFrame` accumulator) advances `displayedContent` one char per ~18ms while there is buffered content (`content.length > displayedContent.length`).
...
The render hook (or a dedicated `useTypewriter(suspectId, messageId)` hook) sets up the requestAnimationFrame loop, calling `tick` when 18ms has accumulated since last tick.
...
**Shipped scope: rain only.** The room-tone half was descoped during Task 7's pre-task asset hunt — see §12 row 7 and §14 for the deviation history. The audio controller is single-element; adding a second loop later is a small additive change.

- **Rain ambient** — gentle rain on window, no thunder, ~30–60 second loop, MP3 (shipped file is larger than the original ≤200KB target; treated as acceptable since rain is the only loop). Source + license recorded in `docs/audio-credits.md`.
- **~~Room tone~~** — *descoped from Task 7; can return in W5+ if playtesting wants more body in the mix.*
...
Only to the **latest** assistant message in the conversation (not player messages, not static UI, **not older assistant replies**). Specifically:
- Wrap only the most recent assistant message's text content in `<JitteringText>` keyed on the active suspect's nervousness. Older assistant messages render plain — they were recorded at earlier nervousness levels and shouldn't retroactively start shaking when the player presses a trigger word now.
- The wrapper updates as the message streams; the entire latest bubble jitters together
- Once a message is fully rendered AND finished streaming AND another assistant reply arrives, the previous one stops jittering — the new latest message takes over. Originally §7.5 wrapped every assistant bubble; revised during Task 5 manual QA — see §12 row 5 for the deviation.
...
| 3 | Char-by-char typewriter rendering | Done | [#34](https://github.com/AlexGalyas/the-ai-interrogation/pull/34) | src/features/interrogation/use-typewriter.ts, src/features/interrogation/chat-message/chat-message.tsx, src/features/interrogation/chat-message/index.ts, src/features/interrogation/chat-view/chat-view.tsx | both | Extracted per-message rendering into `chat-message/` (per ADR-0006 folder + barrel) so the per-message `useTypewriter` hook has a stable component identity to mount under — `chat-view` previously inlined message rendering, which would have violated rules of hooks if we added a hook per iteration. `useTypewriter` is in the feature folder (`src/features/interrogation/use-typewriter.ts`) since it touches store state and is interrogation-specific, not a general primitive. Refresh-snap implementation (per spec §3.3): on hook mount, if `!isStreaming && displayedContent < content`, call `skipTypewriter` immediately rather than replay typewriter or leave the message truncated. RAF loop accumulates wall-clock time and may fire multiple `tick`s per frame if the loop fell behind a 45ms cycle. Playwright happy-path E2E now takes 3.9s (was 1.1s) because the typewriter delays the "working on a piece" assertion — well within the existing 5s timeout, no bump needed. Manual QA verified all five §3.3 cases against the dev server with the real Anthropic API: char-by-char visible, Skip flushes to full content, user messages render instantly, refresh mid-typewriter snaps to full (verified by checking `displayedLen === contentLen` post-reload), each new question gets a fresh typewriter cycle. Pre-existing concern noted: `isStreamingBySuspect` is persisted by the store but never reset on hydration, so a refresh during an in-flight fetch would leave stale `isStreaming=true`. Out of scope for Task 3; would have masked the refresh-snap behavior in that narrow window. |
| 4 | Grain overlay | Done | [#35](https://github.com/AlexGalyas/the-ai-interrogation/pull/35) | src/components/grain-overlay/grain-overlay.tsx, src/components/grain-overlay/index.ts, src/app/globals.css, src/app/layout.tsx | both | Component is a single decorative `<div aria-hidden="true" className="grain-overlay" />` per ADR-0006 (folder + barrel). CSS lives in `globals.css` next to the other dot-pulse keyframes since `mix-blend-mode`, `opacity`, and the inline SVG data URL belong with the global theme layer, not a CSS module. Opacity locked at `0.06` (low end of §4.1's 5–8% range) — Task 8 sensory QA can dial up if too subtle. Mounted in `layout.tsx` just inside `<body>` before `{children}`, so every screen (Briefing, Investigation, Outcome) inherits the layer with no per-screen wiring. All automatic checks green. Manual QA approved by maintainer (2026-05-10): grain visible-but-subtle on dark surfaces across all three screens, text remained fully readable, `pointer-events: none` confirmed via normal clicks/selection working everywhere. |
| 5 | Text jitter mechanic | Done | [#36](https://github.com/AlexGalyas/the-ai-interrogation/pull/36) | src/content/cases/case-01-soho-gallery.ts, src/components/jittering-text/jittering-text.tsx, src/components/jittering-text/index.ts, src/app/globals.css, src/features/interrogation/chat-message/chat-message.tsx, src/features/interrogation/chat-view/chat-view.tsx, src/features/interrogation/use-typewriter.ts | both | Suspect data: Marcus/Henry/Diana got `nervousnessTriggers` verbatim per §7.2 — Marcus 15, Henry 25, Diana 10. `JitteringText` is a pure presentational wrapper: takes `{ children, nervousness }`, picks one of `.jitter-low` / `.jitter-mid` / `.jitter-high` per the §7.4 bands, returns children unwrapped below 20. CSS bands and the `prefers-reduced-motion` override live in `globals.css` next to grain and dot-pulse keyframes. **Two deviations from §3.1 / §7.5 — both originated in this task's manual QA and folded into the spec text inline:** (a) Typewriter interval dropped from 45 → 25 → 18ms (≈55 chars/sec) — 45ms felt sluggish on long Henry replies, 25 was better but still draggy, 18 reads as fast typewriter without losing the per-char reveal. §3.1 + §3.2 spec text updated to 18ms. (b) Jitter scope narrowed from "every assistant message" to "only the latest assistant message" — older replies were recorded at earlier nervousness levels and shouldn't retroactively shake when the player presses a trigger word now. `ChatView` now computes `lastAssistantIndex` and passes `isLatestAssistant` to `ChatMessage`; only the latest one wraps in `<JitteringText>`. §7.5 updated to describe latest-message-only semantics. All four automatic checks green (E2E 2.8s, faster than Task 3's 3.9s because of the 18ms typewriter). Manual QA approved by maintainer (2026-05-10). No tests added for `JitteringText` — pure render-decision logic visible via manual QA; if regressions surface later, `pickJitterClass` is straightforward to unit-test in isolation. |
| 6 | Modal scale-in animation | Done | [#37](https://github.com/AlexGalyas/the-ai-interrogation/pull/37) | src/features/accusation/accusation-modal/accusation-modal.tsx | both | `motion` (v12) was already in `package.json` from a prior install (`pnpm add motion` was a no-op this round); ADR-0016 already authorised the dependency. First **import** of the library lives in this PR — narrowly scoped to the accusation modal as the ADR requires. Took the §5.4 Radix-direct path (NOT the custom-modal fallback): `DialogPrimitive.Root` → `AnimatePresence` → `DialogPrimitive.Portal forceMount` → `DialogPrimitive.Overlay asChild forceMount` wrapping a `motion.div` (backdrop, 300ms blur 0→4px + opacity 0→1) → `DialogPrimitive.Content asChild forceMount` wrapping a `motion.div` (modal, 400ms scale 0.85→1 + opacity 0→1, exit drops to scale 0.95). `cubic-bezier(0.16, 1, 0.3, 1)` per §5.2. Bypassing shadcn's `DialogContent` wrapper means the X close button it provides is gone — close happens through Cancel button / Escape key / backdrop click (all native Radix behaviours). Decision: kept it without the X — the Cancel button is the explicit primary close affordance and the modal is small enough that no second close target is needed; if maintainer disagrees, an X can be added by hand inside the motion.div. All four automatic checks green; manual QA approved by maintainer (entry scale-in feels theatrical, backdrop blur lands first, exit feels lighter, GPU-smooth, sanity-passing happy-path Win flow). |
| 7 | Ambient audio + toggles | Done | [#38](https://github.com/AlexGalyas/the-ai-interrogation/pull/38) | public/audio/rain-loop.mp3, docs/audio-credits.md, src/lib/use-persistent-toggle.ts, src/lib/persistent-toggle-keys.ts, src/components/audio-toggle/audio-toggle.tsx, src/components/audio-toggle/index.ts, src/components/motion-toggle/motion-toggle.tsx, src/components/motion-toggle/index.ts, src/components/audio-controller/audio-controller.tsx, src/components/audio-controller/index.ts, src/app/globals.css, src/features/game-root/game-root/game-root.tsx, src/features/briefing/briefing-screen/briefing-screen.tsx | both | **Deviation — rain only, no room tone.** Maintainer descoped the room-tone half during the §10 pre-task asset hunt; rain alone reads as "atmosphere" cleanly. `AudioController` is structured around a single `<audio>` element so adding a second loop is a small additive change if W5+ wants more body in the mix. §6.1 + this row record the deviation. **Implementation notes:** persistent toggles backed by `useSyncExternalStore` over localStorage (`src/lib/use-persistent-toggle.ts`) — SSR-safe, same-tab and cross-tab updates fan out via an in-process subscriber bus + the native `storage` event; dedicated localStorage keys per spec §6.2 / §7.6 (`the-ai-interrogation:audio:enabled`, `the-ai-interrogation:reduceMotion:enabled`), NOT in the game store. **Reload autoplay fallback:** browsers drop autoplay authorisation across page reloads even when `audioEnabled=true` in localStorage; when `rain.play()` rejects, the controller registers one-shot `click` / `keydown` / `pointerdown` / `touchstart` listeners on the document so the next user gesture transparently resumes the loop. **Reduce Motion `!important`:** §7.6's override only consistently beats the base `.jitter-*` `animation` shorthand under Tailwind v4's layering when marked `!important`; spec §7.6 already suggested it literally and Task 7 manual QA confirmed plain specificity wasn't enough. All four automatic checks green; manual QA approved by maintainer (toggle persists, gesture fallback works after reload, Reduce Motion stops trembling text). |
