...
.grain-overlay {
  ...
  pointer-events: none;
  opacity: 0.10;  /* bumped during Task 8 sensory QA from the original 5–8% range — `0.06` read as invisible on maintainer's display. See §12 row 8 for the deviation. */
...
| 8 | Manual sensory QA + adjustments | Done | [#39](https://github.com/AlexGalyas/the-ai-interrogation/pull/39), [#40](https://github.com/AlexGalyas/the-ai-interrogation/pull/40) | docs/specs/weekend-4-atmosphere.md, docs/journal/weekend-4.md (closure docs in this branch); code fixes tracked in the linked fix-PRs | manual | Full §9.3 QA pass against the merged-state-of-W4 (Task 7 sitting in PR #38). Maintainer findings: **(1) Skip button "jumped" horizontally as the bubble width grew during typewriter streaming** → fix bundled in PR #39 `fix/investigation-layout`: wrapped bubble + Skip in a column flex so the button anchors below the bubble's bottom-left corner; also restyled Skip larger (`text-sm`, `px-3 py-1.5`). **(2) Grain too subtle at `0.06`** → fix PR #40 `fix/grain-opacity-bump`: bumped to `0.10` (one step above the original 5–8% range); §4.1 spec text and the CSS comment updated. **Bundled into PR #39 (Skip-bubble fix triggered a broader layout review):** sticky header (suspect tabs + Accuse) so tabs stay visible while the chat scrolls; chat is the only scrollable region of the screen now; Accuse moved from `fixed bottom-right` to inline in the header (no longer overlaps message input); message input pinned at the bottom; textarea autosizes from 1 row to `max-h-40` with `resize-none` (no manual drag handle); Send button moved inside the textarea wrapper, anchored bottom-right; global `cursor: pointer` rule for non-disabled buttons (Tailwind v4's preflight no longer applies it). **Items flagged but NOT fixed:** rain MP3 size (1.16MB vs §6.1's <200KB suggestion) — maintainer accepted the trade-off given the rain-only descope (Task 7); room-tone half of §6.1 stays deferred (Task 7 deviation, already documented). Other 5 §9.3 checkpoints (typewriter pace, modal scale-in, audio mix, jitter calibration, Reduce Motion) passed without adjustment. New screen recording per §9.4 — maintainer's responsibility, saved outside repo. |
